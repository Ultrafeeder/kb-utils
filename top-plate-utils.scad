...
include <../keyboard_lib/lib/switches/stabilizers.scad>
	    ...
	    )
	  {
	      ...
	      if(!isencoder) 
		{
		  if(k[0]>=2)
			...
			rotate([0,0,rotation]) cube([holesize,holesize,25], center=true);
			translate([-holesize/2,-holesize/2,-1]) rotate([0,0,rotation]) stab(2);
			}
		  ...
		  if(k[1]>=2)
		    {
		      translate([xsize/2,ysize/2,0]) {
			rotate([0,0,rotation]) cube([holesize,holesize,25], center=true);
			rotate([0,0,rotation+270]) translate([-holesize/2,-holesize/2,-1])  stab(2);
		      }
		    }
